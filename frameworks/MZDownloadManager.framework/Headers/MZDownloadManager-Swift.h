...
@interface MZDownloadManager (SWIFT_EXTENSION(MZDownloadManager)) <NSURLSessionDelegate>
...
+ (NSString * _Nonnull)DownloadCompletedNotif;
SWIFT_CLASS_PROPERTY(@property (nonatomic, class, readonly, copy) NSString * _Nonnull baseFilePath;)
+ (NSString * _Nonnull)baseFilePath;
+ (NSString * _Nonnull)getUniqueFileNameWithPath:(NSString * _Nonnull)filePath;
+ (float)calculateFileSizeInUnit:(int64_t)contentLength;
+ (NSString * _Nonnull)calculateUnit:(int64_t)contentLength;
+ (BOOL)addSkipBackupAttributeToItemAtURL:(NSString * _Nonnull)docDirectoryPath;
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
@end
